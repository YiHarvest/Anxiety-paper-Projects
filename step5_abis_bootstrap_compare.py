...
STEP3_BEST = (
    PROJECT / "output" / "step3_single_six_models" / "best_single_biomarker.csv"
)
STEP4_PERF = (
    PROJECT
    / "output"
...
for display, col_base, pred_df in core_candidates:
    ...
    if prob_col in pred_df.columns:
        ...
        print(
            f"  WARNING: Core model '{display}' (column '{prob_col}') not found – skipping."
        )
...
for display_name in core_models:
    ...
    if len(row) > 0:
        ...
        if match_name == "ABIS_LR":
            ...
            print(
                f"  WARNING: No performance row for {display_name} (lookup={match_name})"
            )
...
for display_name, (probs, source) in core_models.items():
    ...
    print(
        f"  {display_name:25s} ROC_AUC={bs['ROC_AUC']:.4f} [{bs['ROC_AUC_CI_lower']:.4f}, {bs['ROC_AUC_CI_upper']:.4f}]  "
...
ax.set_title(
    "ABIS Distribution by Anxiety Status (Test Set)", fontsize=13, fontweight="bold"
)
for spine in ax.spines.values():
    spine.set_linewidth(0.5)
ax.tick_params(labelsize=10)
...
if abis_row and best_single_bs:
    abis_vs_single = (
        "Yes" if abis_row[0]["ROC_AUC"] > best_single_bs[0]["ROC_AUC"] else "No"
    )

# ABIS vs best six-biomarker
six_models_bs = [
    r for r in bootstrap_rows if "Six" in r["Model"] or "Integrated" in r["Model"]
]
...
for display_name in core_models:
    log.append(
        f"  {display_name}: prob_col={core_prob_col_used.get(display_name, 'N/A')}"
    )
log.append("")
log.append("Bootstrap results:")
for bs in bootstrap_rows:
    log.append(
        f"  {bs['Model']:25s} ROC={bs['ROC_AUC']:.4f} [{bs['ROC_AUC_CI_lower']:.4f}, {bs['ROC_AUC_CI_upper']:.4f}]  "
